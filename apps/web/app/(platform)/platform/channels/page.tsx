import Link from "next/link";
import {
  listPlatformChannelsUseCase,
  listTenantsUseCase,
} from "@/lib/di/container";
import { formatPlatformDate, statusBadgeClass } from "@/lib/platform/format";
import type { ChannelConnectionStatus } from "@hcp/domain";

const STATUSES: ChannelConnectionStatus[] = [
  "draft",
  "pending_auth",
  "active",
  "paused",
  "error",
  "disconnected",
];

type SearchParams = Promise<Record<string, string | string[] | undefined>>;

export default async function PlatformChannelsPage({
  searchParams,
}: {
  searchParams: SearchParams;
}) {
  const sp = await searchParams;
  const q = typeof sp.q === "string" ? sp.q : "";
  const tenantId = typeof sp.tenantId === "string" ? sp.tenantId : "";
  const provider = typeof sp.provider === "string" ? sp.provider : "";
  const statusRaw = typeof sp.status === "string" ? sp.status : "";
  const status = STATUSES.includes(statusRaw as ChannelConnectionStatus)
    ? (statusRaw as ChannelConnectionStatus)
    : undefined;
  const page = Math.max(
    1,
    Number(typeof sp.page === "string" ? sp.page : "1") || 1,
  );

  const [tenantsResult, channelsResult] = await Promise.all([
    listTenantsUseCase.execute({ page: 1, limit: 100 }),
    listPlatformChannelsUseCase.execute({
      page,
      limit: 50,
      q: q || undefined,
      tenantId: tenantId || undefined,
      provider: provider || undefined,
      status,
    }),
  ]);

  const tenants = tenantsResult.isSuccess
    ? tenantsResult.getValue().data.map(({ tenant }) => ({
        id: tenant.id,
        name: tenant.name,
      }))
    : [];

  if (channelsResult.isFailure) {
    return (
      <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
        Unable to load channel connections.
      </div>
    );
  }

  const pageData = channelsResult.getValue();

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-[var(--platform-ink)]">
          Channels
        </h2>
        <p className="mt-1 text-sm text-[var(--platform-muted)]">
          Global Channel Manager connections — read-only operational directory.
        </p>
      </div>

      <form
        method="get"
        className="grid gap-3 rounded-md border border-[var(--platform-border)] bg-[var(--platform-card)] p-4 sm:grid-cols-2 lg:grid-cols-5"
      >
        <label className="block text-xs font-medium text-[var(--platform-muted)]">
          Search
          <input
            name="q"
            defaultValue={q}
            placeholder="Name, id, provider"
            className="mt-1 w-full rounded-md border border-[var(--platform-border)] px-2.5 py-1.5 text-sm"
          />
        </label>
        <label className="block text-xs font-medium text-[var(--platform-muted)]">
          Tenant
          <select
            name="tenantId"
            defaultValue={tenantId}
            className="mt-1 w-full rounded-md border border-[var(--platform-border)] px-2.5 py-1.5 text-sm"
          >
            <option value="">All tenants</option>
            {tenants.map((t) => (
              <option key={t.id} value={t.id}>
                {t.name}
              </option>
            ))}
          </select>
        </label>
        <label className="block text-xs font-medium text-[var(--platform-muted)]">
          Provider
          <input
            name="provider"
            defaultValue={provider}
            placeholder="e.g. ical"
            className="mt-1 w-full rounded-md border border-[var(--platform-border)] px-2.5 py-1.5 text-sm"
          />
        </label>
        <label className="block text-xs font-medium text-[var(--platform-muted)]">
          Status
          <select
            name="status"
            defaultValue={status ?? ""}
            className="mt-1 w-full rounded-md border border-[var(--platform-border)] px-2.5 py-1.5 text-sm"
          >
            <option value="">All statuses</option>
            {STATUSES.map((s) => (
              <option key={s} value={s}>
                {s}
              </option>
            ))}
          </select>
        </label>
        <div className="flex items-end">
          <button
            type="submit"
            className="w-full rounded-md bg-[var(--platform-accent)] px-3 py-2 text-sm font-semibold text-white hover:opacity-95"
          >
            Filter
          </button>
        </div>
      </form>

      <p className="text-xs text-[var(--platform-muted)]">
        {pageData.total} connection{pageData.total === 1 ? "" : "s"}
      </p>

      {pageData.data.length === 0 ? (
        <p className="rounded-md border border-[var(--platform-border)] bg-[var(--platform-card)] px-4 py-8 text-center text-sm text-[var(--platform-muted)]">
          No channel connections match these filters.
        </p>
      ) : (
        <div className="overflow-x-auto rounded-md border border-[var(--platform-border)] bg-[var(--platform-card)]">
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-[var(--platform-border)] bg-[var(--platform-muted-bg)]/60 text-[11px] uppercase tracking-wide text-[var(--platform-muted)]">
              <tr>
                <th className="px-4 py-2.5 font-medium">Connection</th>
                <th className="px-4 py-2.5 font-medium">Provider</th>
                <th className="px-4 py-2.5 font-medium">Tenant</th>
                <th className="px-4 py-2.5 font-medium">Status</th>
                <th className="hidden px-4 py-2.5 font-medium md:table-cell">
                  Mappings
                </th>
                <th className="hidden px-4 py-2.5 font-medium lg:table-cell">
                  Updated
                </th>
              </tr>
            </thead>
            <tbody>
              {pageData.data.map((row) => (
                <tr
                  key={`${row.tenantId}:${row.connectionId}`}
                  className="border-b border-[var(--platform-border)] last:border-0"
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/platform/channels/${row.tenantId}/${row.connectionId}`}
                      className="font-medium text-[var(--platform-accent)] hover:underline"
                    >
                      {row.displayName}
                    </Link>
                    <div className="mt-0.5 font-mono text-[11px] text-[var(--platform-muted)]">
                      {row.connectionId}
                    </div>
                    {row.lastError ? (
                      <div className="mt-1 max-w-xs truncate text-xs text-red-700">
                        {row.lastError}
                      </div>
                    ) : null}
                  </td>
                  <td className="px-4 py-3 capitalize">{row.provider}</td>
                  <td className="px-4 py-3">
                    <Link
                      href={`/platform/tenants/${row.tenantId}`}
                      className="text-[var(--platform-accent)] hover:underline"
                    >
                      {row.tenantName}
                    </Link>
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`rounded px-2 py-0.5 text-xs font-medium capitalize ${statusBadgeClass(row.status)}`}
                    >
                      {row.status}
                    </span>
                  </td>
                  <td className="hidden px-4 py-3 tabular-nums md:table-cell">
                    {row.mappingCount}
                  </td>
                  <td className="hidden px-4 py-3 text-[var(--platform-muted)] lg:table-cell">
                    {formatPlatformDate(row.updatedAt)}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}

      {pageData.total > pageData.limit ? (
        <div className="flex gap-3 text-sm">
          {page > 1 ? (
            <Link
              href={`/platform/channels?${new URLSearchParams({
                ...(q ? { q } : {}),
                ...(tenantId ? { tenantId } : {}),
                ...(provider ? { provider } : {}),
                ...(status ? { status } : {}),
                page: String(page - 1),
              }).toString()}`}
              className="text-[var(--platform-accent)] hover:underline"
            >
              Previous
            </Link>
          ) : null}
          {page * pageData.limit < pageData.total ? (
            <Link
              href={`/platform/channels?${new URLSearchParams({
                ...(q ? { q } : {}),
                ...(tenantId ? { tenantId } : {}),
                ...(provider ? { provider } : {}),
                ...(status ? { status } : {}),
                page: String(page + 1),
              }).toString()}`}
              className="text-[var(--platform-accent)] hover:underline"
            >
              Next
            </Link>
          ) : null}
        </div>
      ) : null}
    </div>
  );
}
