 ...
 * Kept free of node:crypto so @hcp/domain remains usable from Next.js bundles.
 */
export function sha256HexUtf8(message: string): string {
  return sha256HexBytes(utf8Encode(message));
}

/** Pure SHA-256 hex digest of raw bytes (lowercase). */
export function sha256HexBytes(message: Uint8Array): string {
  const hash = sha256(message);
  return Array.from(hash, (b) => b.toString(16).padStart(2, "0")).join("");
}

/** Pure SHA-256 digest bytes (32). */
export function sha256Bytes(message: Uint8Array): Uint8Array {
  return sha256(message);
}

/** UTF-8 encode without Node/DOM hard dependency. */
export function encodeUtf8(value: string): Uint8Array {
  return utf8Encode(value);
}

/** UTF-8 byte length (not JavaScript string `.length`). */
